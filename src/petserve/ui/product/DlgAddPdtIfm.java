...
public class DlgAddPdtIfm extends JDialog implements ActionListener {
	...
	private JLabel labelPdtName = new JLabel("商品名称：");
	private JLabel labelBrand = new JLabel("商品品牌：");
	private JLabel labelPdtPrice = new JLabel("商品定价：");
	private JLabel labelPdtCode = new JLabel("商品条码：");
	private JLabel labelPrice = new JLabel("商品售价：");
		...
		if(e.getSource()==this.btnCancel)
			...
			try {
				...
				if(this.edtPdtPrice.getText()==null || "".equals(this.edtPdtPrice.getText()))
					throw new BaseException("商品定价不能为空");
				if(this.edtPrice.getText()==null || "".equals(this.edtPrice.getText()))
					throw new BaseException("商品售价不能为空");
